/* =====================================================
   ...
   IdeaForge Business Intelligence Engine
   ===================================================== */
...
function saveApiKey() {
  ...
  if (key.length < 20) {
    showToast('⚠️ Please enter a valid access key');
    return;
  }
...
async function generateIdeas() {
  ...
  try {
    ...
    if (!ideas || ideas.length === 0) {
      throw new Error('No ideas returned. Please try again with more specific inputs.');
    }
...
// ─── API CALL ─────────────────────────────────────────
async function callClaude(apiKey, prompt, attempt = 1) {
  // IdeaForge Intelligence Engine
  ...
  if (!response.ok) {
    ...
    if (response.status === 401)
      throw new Error('Invalid API key. Tap the ⚙️ settings button and re-enter your access key.');
    if (response.status === 429 && attempt <= 3) {
      const waitSecs = attempt * 15;
      await countdownRetry(waitSecs, attempt);
      return callClaude(apiKey, prompt, attempt + 1);
    }
    if (response.status === 429)
      throw new Error('Rate limit hit. Please wait 1 minute and tap Try Again.');
...
function countdownRetry(seconds, attempt) {
  return new Promise(resolve => {
    const msgEl = document.getElementById('loadingMsg');
    let remaining = seconds;
    const tick = () => {
      if (msgEl) msgEl.textContent = `Rate limit — auto-retrying in ${remaining}s (attempt ${attempt}/3)…`;
...
function parseIdeas(rawText) {
  ...
  throw new Error('Could not process results. Try again with clearer inputs.');
}
