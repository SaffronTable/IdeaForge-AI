...
function saveApiKey() {
  ...
  if (!key.startsWith('AIza') && key.length < 20) {
    showToast('⚠️ Please enter a valid Google API key (starts with AIza)');
    return;
  }
...
async function callClaude(apiKey, prompt) {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      if (response.status === 400) errMsg = 'Invalid API key. Please reset and enter your Google Gemini key.';
